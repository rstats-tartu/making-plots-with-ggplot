---
title: "ggplot2 beyond scatterplot"
author: "Taavi Päll"
date: "2018-10-02"
...
- Using different colours allows visual inference of the distribution of the groups under comparison. 

- But there is apparent limit how much of such information can be accommodated onto one graph before it gets too cluttered.

- In addition to reducing visual clutter and overplotting, we can use small subplots just as an another way to bring out subsets from our data. 

- Series of small subplots (multiples) use same scale and axes allowing easier comparisons and are considered very efficient design. 

- Fortunately, ggplot has easy way to do this: facet_wrap() and facet_grid() functions split up your dataset and generate multiple small plots arranged in an array. 
...
Here, we plot each class of cars on a separate subplot and we arrange plots into 2 rows by class (Hint: use facet_wrap()):
```{r}

```

To plot combination of two variables drv ~ cyl, we use facet_grid():
```{r}
...
If you want to omit rows or columns in facet_grid() use `. ~ var` or `var ~ .`, respectively (like . ~ cyl).
```{r}

```


And we can release x and y scales when data splits placed onto different facet are in (hugely) different scales:
```{r}

```

## Exercises
1. What happens if you facet on a continuous variable (like displ from mpg)?
...
For instance, to create already familiar x = displ, y = hwy dot plot use geom_point():
```{r}
...
Here, 4 stands for four-wheel drive, f for front-wheel drive, and r for rear-wheel drive (?mpg). 
...
Let's Compose displ ~ hwy plot with drv mapped to group aesthetic:
```{r}

```

Other aesthetic mappings (color, alpha etc) similarily group your data for display but also add by default legend to the plot. 
Update plot: to hide legend, set show.legend to FALSE:
```{r}
ggplot(data = mpg) +
...
Let's create displ ~ hwy scatterplot with geom_smooth:
```{r}
...
Here, let's map color to the class of cars, whereas allow geom_smooth plot only one line:
```{r}

```

Importantly, you can use the same idea to specify different data for each layer!
Let's update previous plot so that we use only data from subcompact class to plot geom_smooth (and for clarity, set se = TRUE):
```{r}
...
Let's (re)create diamond counts per cut plot using diamonds_summarised dataset. Note that here we need to use also y-aesthetic!
```{r}

```

2. You might want to override the default mapping from transformed variables to aesthetics. For example, you might want to display a bar chart of proportions of different cuts (x = cut, y = ..prop..), rather than count:
```{r}
 # set also group = 1 
```
...
Let's create cut ~ depth plot of summary stats (median and min/max):
```{r}
?stat_summary
ggplot(data = diamonds)
```
...
Optional additional arguments can be passed on to the functions trough `fun.args = list()`
...
Try to make this plot without flipping... x-axis labels run into each other! 
...
Download administrative map of Estonia:
```{r}
if (!require("pacman")) install.packages("pacman")
pacman::p_load("sp")
if (!file.exists("gadm36_EST_0_sp.rds")) {
 download.file("https://biogeo.ucdavis.edu/data/gadm3.6/Rsp/gadm36_EST_0_sp.rds", 
              destfile = "gadm36_EST_0_sp.rds") 
}
```

```{r}
est <- read_rds("gadm36_EST_0_sp.rds")
